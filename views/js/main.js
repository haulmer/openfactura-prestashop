...
const sendForm = () => {
    ...
    setTimeout(() => {
			const data = {
				...
				actividad: actividad,
                ajax: true,
                controller: 'AdminOpenFactura',
                action: 'SaveDataOpenFacturaRegistry',
                token: token
			}
...
$('#update-button').click(event => {
	...
	const data = {
        ...
        ajax: true,
        controller: 'AdminOpenFactura',
        action: 'UpdateDataOpenfacturaRegistry',
        token: token
	}
	jQuery.ajax({
		url:'index.php',
...
$(function () {
    $('#openDialog-preview').click(event => {
        event.preventDefault();
        const form = document.getElementById('form1');
        const moduleLink = form.elements['moduleLink'].value;
        // init progress bar
        //http://localhost:8888/prestashop/modules/openfactura/views/img/preview.svg
        const data = {
            moduleLink:moduleLink,
        }
        $.confirm({
			title: 'Logotipo personalizado',
            content: '<div>Vista previa</div> <img class="img-preview" alt="Ver ejemplo" src="'+moduleLink+'"/>',
            boxWidth: '640px',
            useBootstrap: false,
